...
const MarketAnalysisPage = ({ onOpenPortal }) => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans selection:bg-amber-500 selection:text-slate-950 pb-12 sm:pb-20 overflow-hidden relative">
      <SeoHead
      ...
      <section className="relative pt-6 pb-10 sm:pt-14 sm:pb-16 px-3 sm:px-6 lg:px-8 border-b border-slate-800 bg-[#050E1A] text-white">
        <FinancialBackground />

        {/* Glow Spheres */}
        <div className="absolute top-0 right-1/4 w-[300px] sm:w-[450px] h-[300px] sm:h-[450px] bg-amber-500/10 rounded-full blur-[120px] pointer-events-none -translate-y-1/2" />
        ...
        <div className="max-w-7xl mx-auto flex flex-col items-center text-center relative z-10">
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-1.5 sm:gap-2 px-2.5 py-1 sm:px-4 sm:py-1.5 rounded-full border border-amber-500/30 bg-[#0B192C] text-amber-400 text-[9px] sm:text-xs font-mono font-extrabold tracking-widest uppercase mb-3.5 sm:mb-5 shadow-xl backdrop-blur-md"
          >
            <Activity className="w-3 h-3 sm:w-4 sm:h-4 text-amber-400 animate-pulse shrink-0" />
          ...
          <motion.h1
            ...
            className="text-xl xs:text-2xl sm:text-5xl md:text-6xl font-black tracking-tight font-heading max-w-4xl leading-tight text-white"
          >
            Daily Market{' '}
      ...
      <main className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 mt-5 sm:mt-8 space-y-5 sm:space-y-10 relative z-10">
        ...
        >
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto whitespace-nowrap scrollbar-none pb-0.5 sm:pb-0 min-w-0">
            <span className="text-[10px] sm:text-xs font-mono font-extrabold mr-1 flex items-center gap-1 text-amber-700 shrink-0">
            ...
            {availableMarkets.map((m) => (
              <button
                ...
                className={`px-2.5 py-1 sm:px-4 sm:py-2 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-black transition-all cursor-pointer shrink-0 ${
                  selectedMarket === m
                    ? 'bg-[#0B192C] text-amber-400 border border-amber-500/40 shadow-md scale-105'
          ...
          <div className="flex items-center justify-between sm:justify-end gap-2 shrink-0 pt-1.5 sm:pt-0 border-t sm:border-t-0 border-slate-100">
            <span className="text-[10px] sm:text-xs font-mono font-bold flex items-center gap-1 text-slate-600">
              <Calendar className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-amber-600 shrink-0" /> Date:
            </span>
            <select
              value={selectedDate}
              onChange={(e) => setSelectedDate(e.target.value)}
              className="bg-slate-50 border border-slate-200 text-slate-900 font-mono font-extrabold text-[10px] sm:text-xs rounded-lg sm:rounded-xl px-2 py-1 sm:px-3.5 sm:py-2 outline-none cursor-pointer transition-colors shadow-inner focus:border-amber-500"
            >
        ...
        {loading ? (
          <div className="py-12 sm:py-20 text-center text-xs sm:text-sm font-mono text-amber-600 animate-pulse flex flex-col items-center justify-center gap-2.5">
            <RefreshCw className="w-6 h-6 sm:w-8 sm:h-8 animate-spin text-amber-600" />
          ...
          >
            <Info className="w-8 h-8 sm:w-12 sm:h-12 text-amber-600 animate-pulse" />
                ...
                >
                  ...
                  <div className="p-3.5 sm:p-6 rounded-2xl sm:rounded-3xl border border-slate-200/90 bg-white flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 shadow-xl">
                    <div className="flex items-center gap-2.5 sm:gap-3">
                      <div className="p-2 sm:p-3 rounded-xl sm:rounded-2xl bg-amber-50 border border-amber-200 text-amber-600 shadow-xs shrink-0">
                        <BarChart3 className="w-4 h-4 sm:w-6 sm:h-6 text-amber-600" />
                      </div>
                      <div>
                        <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
                          <h2 className="text-lg sm:text-2xl font-black tracking-tight font-heading text-[#0B192C]">
                            {item.market}
                          </h2>
                    ...
                    <div className="w-full sm:w-auto grid grid-cols-3 gap-1.5 px-2.5 py-1.5 sm:px-4 sm:py-2.5 rounded-xl sm:rounded-2xl border border-slate-200 bg-slate-50 font-mono text-[10px] sm:text-xs shadow-inner">
                      ...
                      <div className="flex flex-col text-center">
                        <span className="text-[8.5px] sm:text-[10px] font-bold text-amber-700 uppercase">Pivot</span>
                        <div className="font-black text-amber-600 text-[11px] sm:text-sm mt-0.5">
                  ...
                  {item.notes && (
                    <motion.div
                      ...
                      className="p-3.5 sm:p-5 rounded-2xl sm:rounded-3xl border border-amber-200/90 bg-amber-50/90 text-slate-800 flex flex-col gap-1.5 text-xs leading-relaxed shadow-md"
                    >
                      <div className="flex items-center gap-1.5 font-mono font-extrabold text-amber-800 uppercase tracking-wider text-[9.5px] sm:text-[11px]">
                        <Zap className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-600 animate-bounce shrink-0" />
          ...
          >
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1 sm:gap-2">
              <div className="flex items-center gap-2 sm:gap-2.5">
                <div className="p-1.5 sm:p-2 rounded-lg sm:rounded-xl bg-amber-50 border border-amber-200 text-amber-600 shrink-0">
                  <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-amber-600" />
                </div>
                <h2 className="text-base sm:text-2xl font-black tracking-tight font-heading text-[#0B192C]">
                ...
                return (
                  <motion.button
                    ...
                    className={`p-2.5 sm:p-4 rounded-xl sm:rounded-2xl border flex flex-col items-center justify-center gap-0.5 transition-all cursor-pointer ${
                      isActive
                        ? 'bg-[#0B192C] border-amber-500/40 text-amber-400 shadow-lg scale-105 font-bold'
                        : 'bg-white border-slate-200 text-slate-700 hover:border-amber-500 hover:bg-amber-50/50 shadow-xs'
                    }`}
                  >
        ...
        <motion.section
          ...
          viewport={{ once: true }}
          transition={{ duration: 0.4 }}
          className="p-4 sm:p-8 rounded-2xl sm:rounded-3xl border border-slate-800 bg-[#050E1A] text-white flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 sm:gap-6 shadow-2xl relative overflow-hidden"
        >
          <div className="flex items-start sm:items-center gap-3 sm:gap-4 text-left relative z-10">
            <div className="p-2 sm:p-3.5 rounded-xl sm:rounded-2xl bg-amber-500/20 border border-amber-500/40 text-amber-400 shrink-0 shadow-lg animate-pulse">
              <ShieldCheck className="w-5 h-5 sm:w-8 sm:h-8" />
            </div>
            <div className="flex flex-col">
              <h3 className="text-sm sm:text-xl font-black tracking-tight font-heading text-white">
          ...
          <button
            ...
            className="w-full md:w-auto px-4 sm:px-6 py-2.5 sm:py-3.5 rounded-xl sm:rounded-2xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-black text-[11px] sm:text-xs transition-all shadow-xl hover:shadow-amber-500/20 shrink-0 flex items-center justify-center gap-2 cursor-pointer active:scale-95 relative z-10"
          >
